...
- `derivePersonalizationContext(user: User)`: Pure function deriving all four fields from User profile
  ...
  - fedDistrictLabel: STATE_TO_DISTRICT lookup to DISTRICT_NAMES mapping
...
**Commit:** `test(28-audience-shell-separation): add personalization service tests` (1d9bd11)
...
- TypeScript compiles without errors in plan target files
- customer-nav.tsx and mobile-nav.tsx still exist unchanged

**Commit:** `feat(28-audience-shell-separation): create ConsumerNav and wire into public layout` (e3f5143)

## Verification

All criteria met:
- All 9 personalization tests pass (45 total tests passing in vitest run)
- TypeScript compilation: zero errors in plan target files
...
- Existing customer-nav.tsx preserved and unchanged
...
- src/lib/personalization.ts: FOUND
- src/lib/personalization.test.ts: FOUND
- src/components/consumer-nav.tsx: FOUND
- src/components/consumer-mobile-nav.tsx: FOUND
- src/app/(public)/layout.tsx imports ConsumerNav: VERIFIED
- All tests pass: VERIFIED (9 personalization tests, 45 total)

...
#   modal-kill    Fail if runtime TypeScript/JavaScript can call Modal endpoints.
...
modal_kill() {
  ...
  echo "modal-kill: OK (no runtime Modal endpoint/env references in src/)"
  exit 0
}
...
legacy_kill() {
  ...
  echo "legacy-kill: OK (no retired execution references in runtime src/)"
  exit 0
}
...
case "$SUBCOMMAND" in
  ...
  "")
    echo "Usage: $0 <sqlite-kill|modal-kill|legacy-kill|fee-read-model-kill|script-kill>" >&2
    exit 2
    ;;
  *)

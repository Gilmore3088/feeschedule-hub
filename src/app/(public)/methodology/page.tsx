      ...
      <div style={{ maxWidth: "760px", margin: "0 auto", padding: "64px 24px 96px" }}>
        ...
        <Section
          ...
          body={[
            ...
            "Every median, percentile, and count on the site — on the Fee Index, the National Fee Index, and the state and district reports — is computed the same way, from one canonical benchmark table, so the same category shows the same numbers wherever it appears. A fee is \"priced\" when it has a stated amount greater than zero. The institution count for a category is the number of distinct institutions with at least one priced fee in it. When an institution publishes several tiers of the same fee — a monthly maintenance fee that varies by balance, say — we use that institution's lowest priced tier for the median and percentiles, so one institution's multiple tiers cannot count several times in the distribution. The observation count is the number of distinct priced entries (institution, fee name, amount, frequency, and tier) after removing exact duplicates.",
          ]}
        />

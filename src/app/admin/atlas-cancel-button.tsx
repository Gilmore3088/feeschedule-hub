...
import { cancelAtlasRun } from "./atlas-actions";

export function AtlasCancelButton({ runId }: { runId: number }) {
  ...
  function cancel() {
    ...
    startTransition(async () => {
      const result = await cancelAtlasRun(runId);
      if (!result.success) {
        setError(result.error ?? "The backend could not confirm cancellation");
      }
    });
  }

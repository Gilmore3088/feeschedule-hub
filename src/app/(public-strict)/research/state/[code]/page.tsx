...
import { StateReportSkeleton } from "./state-skeleton";
...
export default async function StateReportPage({ params }: PageProps) {
  ...
  // The notFound()-determining lookup above is a static, synchronous check
  // outside any Suspense boundary, so an unknown state resolves a real 404
  // status before Next flushes any bytes. Everything else (all DB reads)
  // streams behind the skeleton.
  return (
    <Suspense fallback={<StateReportSkeleton />}>
      <StateReportContent stateCode={stateCode} stateName={stateName} />
    </Suspense>
  );
}

async function StateReportContent({
  stateCode,
  stateName,
}: {
  stateCode: string;

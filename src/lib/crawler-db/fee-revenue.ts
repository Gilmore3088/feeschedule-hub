...
export function getFeeRevenueData(): FeeRevenueCorrelation[] {
  const db = getDb();
  return db
    .prepare(
      `SELECT
...
export function getTierFeeRevenueSummary(): TierFeeRevenueSummary[] {
  const db = getDb();
  return db
    .prepare(
      `SELECT
...
export function getCharterFeeRevenueSummary(): CharterFeeRevenueSummary[] {
  const db = getDb();
  return db
    .prepare(
      `SELECT
    ...
    .all() as CharterFeeRevenueSummary[];
}

...
// ── InstitutionRevenueQuarter type ────────────────────────────────────────────

describe("InstitutionRevenueQuarter type", () => {
  it("has required fields with correct types", () => {
    const q: InstitutionRevenueQuarter = {
      quarter: "2024-Q4",
      service_charge_income: 500_000,
      fee_income_ratio: 12.5,
      yoy_change_pct: 3.2,
    };
    expect(q.quarter).toBe("2024-Q4");
    expect(q.service_charge_income).toBe(500_000);
  });

  it("allows null fee_income_ratio and yoy_change_pct", () => {
    const q: InstitutionRevenueQuarter = {
      quarter: "2024-Q1",
      service_charge_income: 200_000,
      fee_income_ratio: null,
      yoy_change_pct: null,
    };
    expect(q.fee_income_ratio).toBeNull();
    expect(q.yoy_change_pct).toBeNull();
  });
});

// ── PeerRanking type ──────────────────────────────────────────────────────────

describe("PeerRanking type", () => {
  it("has required fields", () => {
    const pr: PeerRanking = {
      institution_name: "First National Bank",
      tier: "community",
      sc_income: 1_000_000,
      sc_rank: 5,
      peer_count: 200,
      peer_median_sc: 750_000,
      fee_income_ratio: 14.2,
      peer_median_fee_ratio: 12.0,
    };
    expect(pr.tier).toBe("community");
    expect(pr.sc_rank).toBe(5);
  });

  it("allows null institution_name, fee_income_ratio, and peer_median_fee_ratio", () => {
    const pr: PeerRanking = {
      institution_name: null,
      tier: "micro",
      sc_income: 50_000,
      sc_rank: 1,
      peer_count: 10,
      peer_median_sc: 40_000,
      fee_income_ratio: null,
      peer_median_fee_ratio: null,
    };
    expect(pr.institution_name).toBeNull();
    expect(pr.fee_income_ratio).toBeNull();
  });
});
